...
public class VideoZoomHelper extends GestureDetector.SimpleOnGestureListener
    ...
    public boolean onScale(CompatibilityScaleGestureDetector detector) {
        ...
        Call currentCall = MainActivity2.core.getCurrentCall();
        if (currentCall != null) {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        Core core = MainActivity2.core;
        if (isCallEstablished(core.getCurrentCall())) {
            if (mZoomFactor > 1) {
    ...
    private boolean isCallEstablished(Call call){
        if (call == null) {
            return false;
        }
